---
title: "Installation"
description: "Install Merit and set up your environment"
---

## Requirements

- Python 3.12 or higher
- pip or uv package manager

## Install with uv (Recommended)

[uv](https://github.com/astral-sh/uv) is a fast Python package installer and resolver:

```bash
# Install uv
curl -LsSf https://astral.sh/uv/install.sh | sh

# Install Merit
uv pip install merit-analyzer
```

## Install with pip

```bash
pip install merit-analyzer
```

## Verify Installation

Check that Merit is installed correctly:

```bash
merit --version
```

You should see the Merit version number.

## Set Up API Keys (Optional)

### For AI Predicates (LLM-as-a-Judge)

If you plan to use AI-powered assertions, you'll need Merit API credentials:

```bash .env
MERIT_API_BASE_URL=https://api.appmerit.com
MERIT_API_KEY=your_merit_api_key_here
```

<Note>
**Get API Access**: [Contact us](mailto:daniel@appmerit.com) to get your Merit API key.

AI predicates (`has_facts`, `has_topics`, etc.) call the Merit cloud service. Regular assertions work without any API keys.
</Note>

### For Error Analyzer (Bring Your Own Key)

If you plan to use Merit Analyzer, you'll need LLM provider credentials:

<CodeGroup>

```bash OpenAI
OPENAI_API_KEY=sk-...
MODEL_VENDOR=openai
INFERENCE_VENDOR=openai
```

```bash Anthropic
ANTHROPIC_API_KEY=sk-ant-...
MODEL_VENDOR=anthropic
INFERENCE_VENDOR=anthropic
```

```bash AWS Bedrock
MODEL_VENDOR=anthropic
INFERENCE_VENDOR=aws
AWS_ACCESS_KEY_ID=...
AWS_SECRET_ACCESS_KEY=...
AWS_REGION=us-east-1
```

</CodeGroup>

The analyzer uses your own LLM credentials directly (no Merit API required).

## Project Setup

Create a new project directory:

```bash
mkdir my-ai-tests
cd my-ai-tests
```

Merit will automatically discover and run test files that:
- Start with `test_` or end with `_test.py`
- Contain functions starting with `merit_`

## Next Steps

<Card
  title="Write Your First Test"
  icon="code"
  href="/first-test"
>
  Create and run your first Merit test
</Card>

